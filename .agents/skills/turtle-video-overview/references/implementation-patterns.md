...
- **対策**:
  - 閉じたカードのサムネイルはタイトルと時刻の2行分（96×56）。時刻行は `0:16.1–0:40.2` と尺、等倍以外・ミュートだけを出し、横スクロールは出さず折り返す。開いたカードはサムネイルを 48×28 に戻し、2行目の区間表示は隠して従来の編集欄だけを出す。サムネイルは開閉に使わない。省略されたファイル名は PC でマウスオーバー、タッチでタップすると全文を出す（`ClipFileNameButton`）。タッチのタップではカードを開閉しない。
