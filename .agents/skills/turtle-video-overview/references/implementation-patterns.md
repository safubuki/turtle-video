...
### 13-45. リアルタイムWebCodecs時のタイムアウト誤爆と未完成ダウンロードの防止

- **ファイル**: `src/hooks/useExport.ts`
- **問題**:
  - 低スペックPCでエクスポートすると、処理が重いために `processingHardTimeoutMs` （処理全体時間）や `VIDEO_FLUSH_STALL_DETECT_MS` （エンコーダーFlush時の12秒停滞検知）といった独自の終了判断タイムアウトに引っかかる。
  - プログラムが勝手に `controller.abort()` や部分ファイナライズを行い、エンコード中にも関わらず「完了した」とみなしてしまう。
  - その結果、実際には未完成で再生できないMP4ファイルが出来上がり、UI上では「フレーム待機中...」のまま固まったり、未完成なファイルのダウンロードボタンが誤って表示されたりする。
- **対策**:
  - リアルタイムクロック（`loop()`）ベースの映像キャプチャは維持しつつ、**勝手にタイムアウトして未完了ファイルを作る処理をすべて削除**する。
  - `processingHardTimeoutMs` による強制終了（`controller.abort()`）を削除。
  - `flushVideoEncoderWithGuard` 内の `VIDEO_FLUSH_STALL_DETECT_MS` による12秒停止検知を削除。
  - `VIDEO_FLUSH_TIMEOUT_MS` を30秒から180秒（3分）に大幅延長。
- **注意**:
  - これにより、低スペックPCではエクスポートの進行バーが長時間停止したように見えることがあるが、内部でエンコーダーが懸命に処理を続けているため、待てば必ず正しい動画が出力される。不用意にタイムアウトで切り捨てることは、仕様（期待する動画を出力すること）に反する「いい加減な対応」となるため厳禁。
