...
- **対策**:
  - 閉じたカードのサムネイルはタイトルと時刻の2行分（96×56）。時刻はタイトル文言の下へ寄せ、タイトルはサムネイルから少し離す。開いたカードはサムネイルを 48×28 に戻し、2行目の区間表示は隠して従来の編集欄だけを出す。サムネイルは開閉に使わない。省略されたファイル名は PC でマウスオーバー、タッチでタップすると全文を出す（`ClipFileNameButton`）。タッチのタップではカードを開閉しない。
  - フォーカスは `findActiveTimelineItemWithTransitions`（開始以上・終了未満、ディゾルブは後ろ、本編終了以降は最後）。枠は再生中も追従する。
  - 開閉は `ClipsSection` のセッション state。再生位置について開く1枚と、自分で開いたカードを分ける。再生中の連続した進みでは高さもスクロールも変えない。シーク、一時停止、0.5秒以上のジャンプ、1件の追加・複製・続きを追加コピーで追従カードを入れ替える。複数同時追加は最後のカードを開かず、プレビュー位置のカードを開いてそこへスクロールする。
...
- **注意**:
  ...
  - 明示的に閉じた再生位置は、その区間にいる間は開き直さない。別カードへ移って戻ったとき、または別カードまで再生して止めたときに開く。
...
- **注意**: 登録後に左が縦長になるのは意図どおり。揃えるための高さ固定は初期の空状態だけ。
