...
- タイムライン上の開始/終了時間設定
- タイミング打ち（Android/PC）: 無音区間ナビで切れ目へ移動。任意の「読みやすい位置へ自動調整」（既定 OFF）で、ナレーション連動と同じ余白ルール（長い無音は前後 0.1 秒、短い無音は中央で間を空けない）を適用
...
- 縦画面（9:16）時のキャプション／タイトル配置:
  ...
  - 下部プリセットの既定位置はテキスト中心 Y=80%（横画面の端寄りより上）。スマホ UI やセーフエリアで字幕が隠れにくくする
