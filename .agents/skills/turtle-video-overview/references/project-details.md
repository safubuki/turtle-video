...
- **一括音設定アコーディオン**（ロゴ表示の下・standard 限定）: **一括ミュート**、**一括音量**（チェックで有効化、全動画を同じ音量に揃える）、**音量揃え（ノーマライズ）**。合わせ方は **平均に揃える**（RMS の幾何平均へ。小さい音は上げ、大きい音は下げる。極端に小さい動画が多いと目標が小さめに寄る）と **最大に揃える**（最大 RMS へ。小さい音だけ上げ、大きい音はそのまま）から選べる。説明文の文字サイズは揃える。ファイル一覧は表示専用（個別チェックなし）。ファイルが多いときは約5件まで表示し、残りはスクロールする。動画を 1 本や 0 本にしても設定は残し、追加した動画へ一括ミュート・一括音量・音量揃えを引き継ぐ（13-207 / 13-208 / 13-209 / 13-211 / 13-212）
...
- 開始位置（頭出し）、開始タイミング（遅延）
- ボリューム調整、フェードイン/アウト
- **一括音設定**（standard 限定）: BGM とナレーションそれぞれに、動画と同じ「一括ミュート / 一括音量 / 音量揃え（平均に揃える・最大に揃える）」。効く範囲はそのカテゴリだけ。クリップが無くても一括ミュートを先に ON でき、追加直後から無音になる。長い BGM ファイル名でも左カラム幅は広げない（13-211 / 13-212）
