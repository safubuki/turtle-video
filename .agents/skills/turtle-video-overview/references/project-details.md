...
- 画像の表示時間設定（0.5秒〜60秒）
- カードとは独立した範囲指定ウォーターマーク / ブランドロゴ（Issue #210）
  - PNG / JPEG / WebP をプロジェクト時間軸の開始・終了で表示し、複数カードとトランジションをまたいで継続する
  - UI は独立カテゴリーにせず「動画・画像」のカード一覧先頭へ配置し、必要なときだけ展開する
  - 中央位置、自然サイズ 1 倍、透過・回転・ぼかしなしを既定とし、各項目を個別にリセット可能
  - 位置、倍率、透過度、回転、四角・角丸・円形マスク、画像内側へ縮小できるマスクサイズ、周辺ぼかしを調整可能
  - 左下、右下、中央、左上、右上の簡単設定を提供。通常は左右 9/91%、上下 15/85% を基準とし、大きな画像だけ倍率・マスク・回転・ぼかし後の可視領域から中央側へ補正する
  - 非表示中も画像と設定を保持し、プレビュー / 書き出し / 保存復元へ共通反映する
...
- テーマからスクリプト自動生成
- テキスト→音声合成（TTS）
- **場面・状況（全体）**: スタジオ解説 / 商品紹介 / 物語調 / ニュース調 / 自由入力。TTS の監督指示として渡す
- **区間ごとの語り口調**: 原稿は1つの入力欄のまま。範囲選択して「明るく／落ち着いて／はっきり／やわらかく／強調して」または自由入力を適用。原稿内に `《語り口》…《/》` マーカーとして保存（コロン接頭なし。入力欄分割はしない）。音声生成時は Gemini 公式の Audio Profile / Director's Notes + 英語 audio tags へ変換
- AIナレーションカード内から、保持原稿を音声の実効再生区間へ通常キャプションカードとして自動分割・追加（Issue #213）。語り口調マーカーはキャプション本文から除去する
- AIボイス選択: Google AI Studio / Gemini TTS 公式の **30 声**。表示は公式の voice_name・trait と Cloud TTS 公式の性別のみ。UI で「すべて／女性／男性」に絞り込み可能。既定は Aoede
...
### 5. キャプション
- キャプションの追加・編集
- 通常の入力欄には「単発キャプション」の項目名を表示し、まとめて入力・時分割と区別
- スタイル設定（文字サイズ、字体、縁幅、縁色、文字本体色、表示位置、ぼかし、**背景の帯**）。縁幅はスライダーと数値入力、色はカラーピッカーと16進数入力に対応。背景の帯は既定 OFF（黒・透過・角丸はタイトル背景帯と同じ操作感）
- カードごとの個別設定でも文字サイズ、字体、縁幅、縁色、文字本体色、表示位置、ぼかし、背景の帯、フェードを上書き可能。未変更の項目は一括設定を継承
- 新規プロジェクトの既定は黒い縁（キャプション 4px / タイトル 5px）・白い文字本体
- タイムライン上の開始/終了時間設定
- タイミング打ち（Android/PC）: 無音区間ナビで切れ目へ移動。「読みやすい位置へ自動調整」は既定 ON（ナレーション連動と同じ余白ルール。exact にしたいときだけ OFF）
...
- 動画タイトル（Issue #211）: キャプションとは**別管理**の 1 件だけの設定。キャプションセクション先頭の「タイトル」アコーディオン（初期状態は閉じている）から設定する
  ...
  - 見た目は「スタイル設定」アコーディオンにまとまっており、サイズ（小/中/大/特大 + カスタム）・字体（その他▾ / PC の全フォント読み込み対応）・位置（上/中央/下 + XY カスタム）・縁の幅と色・文字色・**ぼかし（0〜5）**・背景の帯（色/濃さ/角丸）を調整できる。サイズと字体はキャプションと同じ UI コンポーネントを共有している
...
- 縦画面（9:16）時のキャプション／タイトル配置:
  - 文字サイズはキャンバス**短辺**を 1080 とみなしてスケールするため、横 16:9 と同程度の大きさになる（高さ基準だと縦で約 1.78 倍になり見づらい）
  - 下部プリセットの既定位置はテキスト中心 Y=80%（横画面の端寄りより上）。スマホ UI やセーフエリアでキャプションが隠れにくくする

### 6. プレビュー & 再生
- Canvas上でのリアルタイムプレビュー
- 再生/一時停止/停止、シークバー
- 連続する画像カードはシークバー上でオレンジ系の濃淡と境界線を表示し、カード区切りを視認できる
- タブ復帰時の自動リフレッシュ
...
- 出力品質（auto / フルHD / HD）とアスペクト比の向き（16:9 横 / 9:16 縦）を選択可能
  ...
  - キャプション・動画タイトルの文字サイズと下部位置も縦向けに調整される（短辺スケール・下部 Y=80%）
- **キャプションのみ出力（Issue #114・Android/PC standard 限定）**
  - プレビューの「動画出力オプション」にサムネイル設定と書き出し設定をまとめ、「完成動画 / キャプションのみ」を選択
  - キャプションのみはベース映像・音声を含めず、キャプション + 動画タイトルだけをオフライン encode
  - キャプションが0件の場合は選択不可。選択後に全削除された場合は完成動画へ自動復帰
  - 形式は利用頻度順に透過 WebM / 黒背景 MP4 / 白文字キー用 MP4（透過非対応時は黒背景へフォールバック）
  - 標準的な MP4（H.264）は alpha 非対応のため、背景透過は WebM のみ
  - 動画生成後はダウンロード対象と設定の不一致を防ぐため、停止または再生で生成物を解除するまで出力オプションをロック
  - 文字は2倍解像度でラスタライズして高品質縮小し、通常の2倍ビットレートでキャプション輪郭と透過境界のジャギー・圧縮荒れを抑える
  - 汎用字幕ファイル（SRT / VTT）を同時または単独ダウンロード可能。単独ダウンロードは字幕設定内に配置（時分割行は個別キューへ展開）
  - 仕様: `Docs/specs/2026-08-01_issue-114-caption-layer-export.md`
- スマホの「動画・画像」一覧は `min(32rem, 72svh)` を上限にし、標準カード1件と次のトランジションを確認しやすくする（PCは従来の `lg:max-h-128`）
...
| `overlayStore` | プロジェクト全体ウォーターマークの画像・時間範囲・見た目を管理 |
