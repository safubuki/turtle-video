...
- テーマからスクリプト自動生成
- テキスト→音声合成（TTS）
- AIナレーションカード内から、保持原稿を音声の実効再生区間へ通常キャプションカードとして自動分割・追加（Issue #213）。改行のない長文にも対応し、Android/PC版では文字数比の境界に近い安全な無音区間を選び、前カードを無音開始で終了・次カードを無音終了から開始して、発話していない間は字幕を表示しない。解析失敗・候補なし・iPhone/iPad版では文字数比へフォールバックし、追加後は各カードの文字と開始・終了を個別編集可能
