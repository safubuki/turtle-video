...
**対応完了状況**: ✅ 主要不具合は修正済み（静止画+BGM含む） / ⏳ 出力尺短縮は追加対策を実装し再検証中
...
### 0.9 追加事象: 出力尺の短縮 / 静止画+BGM無音（iOS Safari）

#### 事象
1. プレビューは約5.9秒だが、保存動画が約5.2秒になる（継続）
...
### 0.10 残課題（5.9秒→5.2秒）に対する次の対応案

1. **停止タイミングを「目標尺 + 安全マージン」に固定する**
   - 現在は描画ループ終了/abort起点の停止だが、iOS Safari では stop 時点で末尾取りこぼしが出る
   - iOS限定で `totalDuration + 200〜350ms` の停止スケジュールを導入し、末尾を確実に収録

2. **`ondataavailable` の最終時刻ベースで停止判定する**
   - チャンク到着間隔を監視し、最終チャンクが目標時刻に達するまで停止を遅延
   - 固定ディレイより端末差への耐性が高い

3. **iOS限定で「尺優先モード」を追加する**
   - 既定は現行挙動、`durationStrict` を有効にした場合のみ停止猶予を長めにとる
   - 音ズレ/ファイルサイズ増加の副作用を運用で制御しやすい

4. **診断ログを拡張して端末差を可視化する**
   - `chunks.length`, 開始〜停止経過ms, 最終 `ondataavailable` 到達時刻を記録
   - 端末/OSごとの差分を定量的に比較し、最適な停止猶予を決める
