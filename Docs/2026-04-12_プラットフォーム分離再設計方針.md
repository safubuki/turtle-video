...
方針書では「Safari 向けのはずの重い前処理が非 Safari 側にも共有されている」と問題指摘している。
...
方針書では `previewPlatform.ts` を「helper」と位置付けているが、実際には次の規模を持つ。
...
方針書のタスク粒度「preview engine を抽出」は大きすぎるため、以下のサブタスクに分解を推奨する。
