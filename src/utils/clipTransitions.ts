 ...
 *
 * 設計方針: ディゾルブだけはクリップを重ね、重なり時間ぶんタイムラインを短縮する。
 * - ディゾルブ: 現クリップが流れ続ける終端 d 秒間へ、次クリップの実フレームを
 *   アルファを上げながら重ねる。総尺と各クリップの区間は transitionTimeline を単一ソースにする。
 * - フェード(黒/白): 境界の前 d/2 秒で色板へディップし、後 d/2 秒で明ける。
 *   フェードでは総再生時間・各クリップの時間帯を変えない。
