...
describe('standard preview engine', () => {
  ...
  it('24fps動画→24fps export は実提示フレーム更新前の次slotを公開しない', async () => {
    const video = createVideoItem({
      id: 'video-rvfc',
      duration: 10,
      originalDuration: 10,
    ...
    Object.assign(videoEl, {
      requestVideoFrameCallback: vi.fn((callback: TestVideoFrameCallback) => {
    ...
    expect(audioSources).toBeDefined();
    audioSources.onAudioPreRenderComplete?.();
    expect(audioSources.getRenderedFrameStats?.().distinctRenderedFrames).toBe(0);

    callbackRef.current?.(0, { mediaTime: 0, presentedFrames: 1 });
    nowMs = 16;
    hook.result.current.loop(true, loopIdRef.current);
    expect(audioSources.getRenderedFrameStats?.()).toMatchObject({
      distinctRenderedFrames: 1,
    ...
    // 壁時計上は次の 24fps slot へ到達しても、実画像の通知が同じなら公開しない。
    nowMs = 60;
    hook.result.current.loop(true, loopIdRef.current);
    expect(audioSources.getRenderedFrameStats?.()).toMatchObject({
      distinctRenderedFrames: 1,
    ...
    videoEl.currentTime = 1 / 24;
    callbackRef.current?.(60, { mediaTime: 1 / 24, presentedFrames: 2 });
    nowMs = 76;
    hook.result.current.loop(true, loopIdRef.current);
    expect(audioSources.getRenderedFrameStats?.()).toMatchObject({
      distinctRenderedFrames: 2,
    ...
    expect(logInfo.mock.calls.some(([, message]) =>
      message === 'standard.export.timeline.videoPresentationPaused')).toBe(true);
    expect(logInfo.mock.calls.some(([, message]) =>
      message === 'standard.export.timeline.videoPresentationResumed')).toBe(true);
  });
