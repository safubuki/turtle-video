...
describe('version metadata', () => {
  it('v5.1.0 の現在バージョンと代表的な変更概要を持つ', () => {
    expect(versionData.version).toBe('5.1.0');
    expect(versionData.history.previousVersion).toBe('4.1.0');
    expect(versionData.history.summary).toContain('iOS Safari');
    expect(versionData.history.summary).toContain('PC/Android');
    ...
    expect(versionData.history.highlights).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ title: 'PC/Android書き出しの滑らかさと境界安定性を改善' }),
      ])
    );
  });
});
