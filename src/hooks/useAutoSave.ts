  ...
  const computeHash = useCallback(() => {
    const parts = [
      ...
      mediaItems.map((m) => [
        ...
        m.thumbnailMode ?? 'auto',
        m.thumbnailSourceTime ?? '',
        m.playbackSpeed ?? 1,
        m.showSpeedBadge ? 1 : 0,
      ...
      captions.map((c) => [
        ...
        c.overrideStrokeWidth ?? '',
        c.overrideBlur ?? '',
        c.overrideBackgroundEnabled ?? '',
        c.overrideBackgroundColor ?? '',
        c.overrideBackgroundOpacity ?? '',
        c.overrideBackgroundRadius ?? '',
        c.overrideFadeIn ?? '',
        c.overrideFadeOut ?? '',
      ...
      JSON.stringify({
        ...
        fadeIn: watermarkOverlay.fadeIn,
