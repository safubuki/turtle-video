      ...
      items: [
        {
          title: 'ウォーターマーク',
          description:
            '動画・画像一覧の先頭にある折りたたみ設定です。必要なときだけ画像を追加し、カードをまたぐ表示範囲、中央基準の位置、自然サイズを1とする倍率、透過度、回転、マスク形状・サイズ、周辺ぼかしを調整できます。位置は「左下、右下、中央、左上、右上」の簡単設定から選べます。四隅は左右9/91%、上下15/85%を基準にし、大きな画像だけ見切れすぎないよう自動調整します。マスクサイズを小さくすると画像外周より内側で自然にぼかせます。各調整値はくるくるアイコンでデフォルトへ戻せます。',
          visuals: ['reset_button', 'slider_demo'],
        },
        {
          title: '追加ボタン',
        ...
        },
        {
          ...
          description:
            '動画は開始・終了位置を指定してトリミングできます。「開始」「終了」ボタンでは、プレビューの現在位置をその動画のトリミング開始・終了へ反映できます（すでにトリミング済みの動画でも、現在の有効区間を基準に計算します）。画像は表示時間を常時調整できます。どちらもスライダーでも操作できます。',
    ...
    bgm: {
      ...
      items: [
        ...
        },
        {
          ...
          description:
            '複数の曲を追加してシーンごとに切り替えられます。タイトル右側の「(n件)」で登録数を確認できます。追加時は動画の長さに合わせて自動配置されます。「動画尺に合わせて自動調整」は既定でONです。ONのときは、各曲の設定（開始・終了）はそのまま残し、再生に使う有効区間だけが自動で調整されます（途中の曲は設定どおり、最後の曲だけ動画末尾まで短縮/延長）。OFFにすると設定した区間のまま再生し、動画が長くなっても延長しません（ここからここまでだけ鳴らしたいとき向け）。動画終端より後から始まる曲は一時無効になり、尺が戻ると復元されます。「設定を末尾に固定」は設定値そのものを書き換えたいときだけ使います。フェードアウトも有効終端を基準にかかります。',
    ...
    narration: {
      ...
      items: [
        ...
        },
        {
          ...
          description:
            'AIナレーションのカード内にあるボタンを押すと、保持している原稿を読みやすい長さに分け、実際のナレーション再生区間へ通常のキャプションカードとして追加します。Android/PC版では0.12秒以上の無音を解析し、0.3秒未満の短い無音は字幕を消さず中央でカードを切り替えます。0.3秒以上の無音では、発話終了後と次の発話開始前に約0.1秒ずつ字幕を残し、中央部分だけ字幕を表示しません。解析できない場合も文字数比で必ず作成し、追加後は各カードの文字と開始・終了を個別に微調整できます。',
      ...
      items: [
        {
          ...
          description:
            'セクションの先頭にある「タイトル」（開いて設定）を押すと、動画タイトルを設定できます。毎回使う機能ではないため初期状態は閉じています。タイトルは通常のキャプションとは別に保存され、キャプション一覧や時分割カードには並びません。タイトル文字の下にある開始・終了は、キャプションカードと同じスライダーと数値で設定でき、「プレビュー位置を反映」の開始/終了ボタンで現在の再生位置を取り込めます。初期値は中央・通常キャプションより大きめの文字で、表示は0〜4秒、開始フェードなし・終了フェード1秒です。見た目は「スタイル設定」（開いて設定）にまとまっており、サイズ、字体、位置、縁の幅・色、文字本体色、タイトル背景の帯（色・濃さ・角丸）を調整できます。変更はすぐプレビューへ反映され、書き出した動画にも同じ見た目で入ります。',
          visuals: ['caption_style_accordion', 'position_chip', 'start_chip', 'duration_chip'],
        },
        {
          title: '追加ボタン',
        ...
        },
        {
          title: '文字の縁・色',
          description:
            '「キャプション スタイル/フェードの一括設定」を開き、字体の下にある「文字の縁・色」（開いて設定）を押します。「縁の幅」はスライダーまたは数値入力で調整でき、「縁の色」と「文字本体」は色見本または#RRGGBB形式で指定できます。初期値は白い文字本体と黒い縁（4px）です。',
        ...
        },
        {
          ...
          description:
            '「キャプションカードをまとめて移動」の枠内で対象を選び、「現在位置に先頭を合わせる」を押すと、対象範囲の最初のキャプションがプレビューの現在位置から始まるように一括移動します。各カードの表示時間と間隔は維持され、終了位置の指定は不要です。従来の秒数指定と「早める」「遅らせる」は微調整に利用できます。動画・ナレーション・BGMは移動しません。',
    ...
    preview: {
      ...
      items: [
        ...
        },
        {
          title: '音量波形と無音区間',
          description:
            'シークバーのすぐ下に、プロジェクト全体の音量変化が波形で表示されます。ナレーションと BGM に加えて、動画そのものに入っている音声も反映されるので、動画だけを取り込んだ場合でも波形が出ます（音声を含まない動画では表示されません）。横幅と時間軸はシークバーと同じなので、波形の位置がそのまま再生位置に対応します。波形をタップするとその位置へ移動できます。黄色い帯は無音区間（発話の切れ目）で、「無音区間：前へ／無音区間：次へ」で境界へ素早く移動できます。移動先には動画の先頭（0秒）と末尾も含まれるので、1つ目のキャプションを動画の先頭に合わせたいときにも使えます。同じボタンはキャプションの「タイミング打ち」バーにもあり、-1s の左と +1s の右に並んでいます。判定の基準はナレーションが最優先で、無ければ動画音声、BGM の順に切り替わります（BGM が流れていても発話の切れ目を見つけられます）。現在どれを基準にしているかは波形の下に表示されます。移動しただけではキャプションの時間は変わりません。移動したあとキャプション側の「現在位置を開始に設定」などで反映してください。音声素材やトリミング、音量を変えると波形は作り直されます。iPhone・iPad では波形は表示されません。',
        },
        {
          title: 'サムネイル（プロジェクト全体）',
          description:
            '複数の動画・画像を1本にまとめた完成映像の代表フレームを、プレビュー下で設定できます。小さいプレビュー画像と「自動／手動」で現在の設定が分かります。既定はタイムライン先頭付近（約0.2秒）です。「現在のフレームをサムネイルに設定」で手動にし、書き出し時に MP4 のカバーアート（標準の動画サムネイル手法）と先頭キーフレームへ反映します。サムネイルは現在の動画形式（横16:9／縦9:16）で保存され、形式を変更して手動画像の比率が合わなくなった場合は自動設定へ戻ります。設定したあと「動画ファイルを作成」してください。OS によってはカバーアートより映像の途中フレームをアイコンに使う場合があります。',
        },
        {
          title: '動画ファイルを作成',
          description:
