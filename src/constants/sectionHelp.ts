    ...
    preview: {
      ...
      items: [
        ...
        },
        {
          ...
          description:
            '複数の動画・画像を1本にまとめた完成映像の代表フレームを、プレビュー下で設定できます。小さいプレビュー画像と「自動／手動」で現在の設定が分かります。既定はタイムライン先頭付近（約0.2秒）です。「現在のフレームをサムネイルに設定」で手動にし、書き出し時に MP4 のカバーアート（標準の動画サムネイル手法）と先頭キーフレームへ反映します。設定したあと「動画ファイルを作成」してください。OS によってはカバーアートより映像の途中フレームをアイコンに使う場合があります。',
        },
        {
          title: '動画ファイルを作成',
          description:
