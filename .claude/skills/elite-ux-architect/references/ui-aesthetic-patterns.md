...
| 破壊的アクション | 赤系。確認ダイアログ必須。**色だけでなくラベルでも破壊性を伝える** |
| 無効状態 | 彩度を落とし、opacity: 0.5。**理由をツールチップで説明する** |
| ホバー | 明度を1段階変化（例: blue-500 → blue-600） |

### パレット確定時は必ず数値検証する

目視で「読めそう」は判断にならない。ライト/ダーク両方を総当たりで検証する：
...
> 📄 OKLCH による知覚的に均等なスケール生成、WCAG 2.x の限界、APCA の正しい位置づけ、色覚多様性対応は **[color-science.md](color-science.md)** を参照。
